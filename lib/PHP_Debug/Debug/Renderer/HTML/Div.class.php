...
wpdutil::loadClass('PHP_Debug.Debug.Renderer.HTML.DivConfig');
...
{    
    ...
    protected function showW3cValidation()
    {
        ...
        if ($this->options['enable_w3c_validator']) {
            ...
            if ($res) {
                if ($res->isValid()) {
                    $results = '<h2><img src="{$imagesPath}/info.png" alt="Valid"/> '.__('The output is valid','wp-pear-debug').'</h2>';
                } else {
                    $results = '<h2><img src="{$imagesPath}/error.png" alt="Not valid" /> '.__('The output is NOT valid','wp-pear-debug').'</h2>';
                    ...
                    if ($res->errors || $res->warnings) {
                      ...
                      $results = '<h2><img src="{$imagesPath}/warning.png" alt="warning" /> '.__('Validation results can\'t be retrieved (localhost source ?)','wp-pear-debug').'</h2>
                      ';
                    }
                }
            } else {
            	throw new exception('Services_W3C_HTMLValidator :'.__('Unable to parse','wp-pear-debug'). 
                    $url);
            }
        } else {
            $results = '';
    ...
    protected function showDatabaseInfos($debugInfos)
    {
        ...
        return str_replace(
            array('{$buffer}'),
            array($buffer ? $buffer : '<li>'.__('No database debug available','wp-pear-debug').'</li>'),
    ...
    */
    ...
    protected function showError($infos)    
    {
        ...
        {
            case E_WARNING:
                $errorlevel = 'PHP '.__('WARNING','wp-pear-debug').' : ';
                $buffer .= '<span class="pd-php-warning"> /!\\ '. 
                    $errorlevel. $infos[1] . ' /!\\ </span>';                
                break;

            case E_NOTICE:
                $errorlevel = 'PHP '.__('notice','wp-pear-debug').' : ';
                $buffer .= '<span class="pd-php-notice">'. 
                    $errorlevel. $infos[1] . '</span>';
                break;

            case E_USER_ERROR:
                $errorlevel = 'PHP '.__('User error','wp-pear-debug').' : ';
            ...
            case E_STRICT:
                ...
                $errorlevel = 'PHP '.__('STRICT error','wp-pear-debug').' : ';
                $buffer .= '<span class="pd-php-user-error"> /!\\ '. 
                    $errorlevel. $infos[1] . ' /!\\ </span>';
                break;

            default:
                $errorlevel = 'PHP '.__('errorlevel','wp-pear-debug').' = '. $infos[0]. ' : ';
                $buffer .= $errorlevel. 
